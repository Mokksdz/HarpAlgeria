...
export default function JournalArticleClient({ post, relatedPosts }: Props) {
  const { t, language } = useLanguage();
  const isAr = language === "ar";

  const CATEGORY_LABELS: Record<string, string> = {
    STYLE: t("journal.style"),
    LOOKBOOK: t("journal.lookbook"),
    TIPS: t("journal.tips"),
    NEWS: t("journal.news"),
  };
  ...
  return (
    <div
      className="min-h-screen bg-gradient-to-b from-harp-cream/30 to-white"
      dir={isAr ? "rtl" : "ltr"}
    >
      {/* Breadcrumb */}
            ...
            <div className="mt-8 pt-8 border-t border-gray-100">
              <div className="flex items-center gap-4">
                ...
                <a
                  ...
                  aria-label={t("journal.shareOnFacebook")}
                >
                  <Facebook size={18} />
                </a>
                <a
                  ...
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-full bg-harp-sand/40 flex items-center justify-center text-harp-brown hover:bg-harp-brown hover:text-white transition-all"
                  aria-label={t("journal.shareOnTwitter")}
                >
                  <Twitter size={18} />
                </a>
                <a
                  ...
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-full bg-harp-sand/40 flex items-center justify-center text-harp-brown hover:bg-harp-brown hover:text-white transition-all"
                  aria-label={t("journal.shareOnWhatsApp")}
                >
                  <svg
                    viewBox="0 0 24 24"
